...
export class PleskSSh {
  ...
  yarnBuild(user:string,commands:string,path:string = 'httpdocs') {
  ...
  prismaMigration(user:string,path:string = 'httpdocs') {
    const login = `su - ${user} -s /bin/bash`
    const move = `cd ${path}`
  ...
  prismaSeeding(user:string,path:string = 'httpdocs') {
    const login = `su - ${user} -s /bin/bash`
    const move = `cd ${path}`
